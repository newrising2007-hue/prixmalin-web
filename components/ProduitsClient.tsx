            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-xl font-semibold">
                  <Link href={`/${locale}/produit/${p.slug}${filtre !== "toutes" ? `?cat=${filtre}` : ""}`} className="hover:underline">{(locale !== "fr" && p[`title_${locale}`]) ? p[`title_${locale}`] : p.title}</Link>
                </h2>
                {p.category && (
                ...
                <p className="mt-3">
                  <Link href={`/${locale}/produit/${p.slug}${filtre !== "toutes" ? `?cat=${filtre}` : ""}`} className="text-sm font-medium text-blue-700 hover:underline">
                    {labels["voir_fiche"]}
                  </Link>
                </p>
              </div>
              <div className="sm:shrink-0 flex flex-col items-end gap-1">
                {p.prixVerifieLe && (
                  <span className="text-xs text-black/40">{labels["verifie_le"]} {p.prixVerifieLe ? new Date(p.prixVerifieLe + "T12:00:00").toLocaleDateString(locale, { year: "numeric", month: "long", day: "numeric" }) : ""}</span>
                )}
