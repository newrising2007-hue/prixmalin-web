...
export function generateStaticParams() {
  const games = normalizeGames(xboxGames);
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Codes ${name} (Xbox) | PrixMalin`;
  const description =
    game?.seoDescription ??
    `Codes ${name} sur Xbox : uniquement des codes réels (quand disponibles), avec source et expiration.`;

  const url = `${SITE_URL}/codes-bonus/xbox/${gameSlug}`;
...
export default async function Page({ params }: PageProps) {
  ...
  const items = codes
    .filter((c: any) => c?.gameSlug === gameSlug && c?.platform === "xbox" && c?.isActive !== false)
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: `Codes ${game.name} (Xbox)`,
    itemListElement: items.slice(0, 30).map((it: any, idx: number) => ({
      "@type": "ListItem",
      position: idx + 1,
      name: it.title ?? it.id,
      url: `/codes-bonus/xbox/${gameSlug}#${it.id}`,
    })),
  };
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 pb-16 pt-10">
      ...
      <nav className="mb-6 text-sm text-neutral-600">
        ...
        <Link className="hover:underline" href={locale === "fr" ? "/codes-bonus/xbox" : `/${locale}/codes-bonus/xbox`}>
          Xbox
        </Link>
        <span className="mx-2">/</span>
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">{t("codes_jeu_xbox", { nom: game.name })} 🇨🇦</h1>
      ...
      {Array.isArray(game.relatedSlugs) && game.relatedSlugs.length > 0 ? (
        <section className="mt-12">
          <h2 className="text-lg font-semibold">{t("autres_jeux_xbox")}</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {game.relatedSlugs.map((s) => (
              <Link
                key={s}
                href={locale === "fr" ? `/codes-bonus/xbox/${s}` : `/${locale}/codes-bonus/xbox/${s}`}
                className="relative rounded-full border border-emerald-200 bg-emerald-50 px-5 py-2 text-sm font-semibold text-emerald-900 shadow-sm shadow-emerald-500/10 transition-all duration-200 hover:bg-emerald-100 hover:shadow-md hover:shadow-emerald-500/20 hover:-translate-y-0.5 hover:ring-2 hover:ring-emerald-200/60 active:translate-y-px active:shadow-sm"
              >
                {titleCaseSlug(s)}
              </Link>
