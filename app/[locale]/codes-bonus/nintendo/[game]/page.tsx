...
export function generateStaticParams() {
  const games = normalizeGames(nintendoGames);
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Codes ${name} (Nintendo) | PrixMalin`;
  const description =
    game?.seoDescription ??
    `Codes ${name} sur Nintendo : uniquement des codes réels (quand disponibles), avec source et expiration.`;

  const url = `${SITE_URL}/codes-bonus/nintendo/${gameSlug}`;
...
export default async function Page({ params }: PageProps) {
  ...
  const items = codes
    .filter((c: any) => c?.gameSlug === gameSlug && c?.platform === "nintendo" && c?.isActive !== false)
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: `Codes ${game.name} (Nintendo)`,
    itemListElement: items.slice(0, 30).map((it: any, idx: number) => ({
      "@type": "ListItem",
      position: idx + 1,
      name: it.title ?? it.id,
      url: `/codes-bonus/nintendo/${gameSlug}#${it.id}`,
    })),
  };
